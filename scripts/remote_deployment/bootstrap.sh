...
# conda env setup success?
if [[ -z "${ENV_NAME:-}" ]]; then
  echo "ERROR: Could not determine conda environment name." >&2
  exit 1 # issue with conda env
fi
echo "Using env: $ENV_NAME" # conda env setup (env name -> std::out)
...
# TODO: reconsider opting to use pyproject.toml to handle deps / requirements.txt
# for now, this is OKAY, so we can easily deploy to a fresh remote instance
...
# Steps 6 & 7
# requirements for reasoning (ROBOT, Protege, DeepOnto, etc)
...
# messy work-around (TODO: better dependency management)
...
# set AUTO_ENV_NAME if not already set:
if ! grep -q '^AUTO_ENV_NAME=' .env 2>/dev/null; then
  echo "AUTO_ENV_NAME=$ENV_NAME" >> .env
fi

# Register the vendor forks in lib and project src \w pyproject.toml
