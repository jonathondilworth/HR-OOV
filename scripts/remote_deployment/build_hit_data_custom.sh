...
echo "[Step 1/2] Building HiT dataset (mixed) for training custom HiT models ... "

echo "" | conda run -n "$AUTO_ENV_NAME" --no-capture-output python ./scripts/load_taxonomy.py \
  --ontology ./data/custom.owl \
  --training-data ./data/hit_dataset_custom
...
conda run -n "$AUTO_ENV_NAME" --no-capture-output python ./scripts/preprocess_entity_lexicon.py \
  --input ./data/hit_dataset_custom/multi/entity_lexicon.json \
  --output ./data/hit_dataset_custom/multi/local_entity_lexicon.json \
  --strip-parens \
  --to-lower-case

conda run -n "$AUTO_ENV_NAME" --no-capture-output python ./scripts/preprocess_entity_lexicon.py \
  --input ./data/hit_dataset_custom/mixed/entity_lexicon.json \
