...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Add entity mentions to each question in a MIRAGE question file"
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        ...
        "-o",
        required=True,
        help="Directory where benchmark questions + entity mentions will be written (JSON/JSONL)",
    )
    # extract HEAD entities or biomedical question entities
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument(
        "--extract-head",
        action="store_true",
        help="Extract with rule-based HEAD extraction",
    )
    mode.add_argument(
        "--extract-biomed",
        action="store_true",
        help="Run biomedical NER with SciSpaCy (must be instaled)",
    )
    # run NER \w ... TODO: support for extra extraction methods
    parser.add_argument(
        "--ner",
        default="scispacy",
...
def load_questions(in_path: Path):
    ...
    if "questions" not in payload:
        raise ValueError("Input JSON object is missing 'questions' key")
...
def extract_head_entities(question: str):
    """process question string for extraction of HEAD entities"""
...
def annotate_questions(questions: Sequence[Dict[str, Any]], *, use_biomed: bool, ner_backend: str) -> None:
    """annotate each MIRAGE question with named entities using ner_backend"""
    if use_biomed: # TODO: implement other methods
